...
class DataPreprocessor:
    ...
    def prepare_data_for_keras_model(
        self, train: pd.DataFrame, test: pd.DataFrame, valid: pd.DataFrame
    ):
        """_summary_
        ...
        self._logger.info(
            f"There are {vocab_size} words in the corpus, it will be set to {self.max_vocab_size} based on the config file"
        )
        # reducing vocab size
        ...
        self._logger.info(
            f"The max lenght is set to {self.max_length} based on the data analysis"
        )
        ...
        for data in train, test, valid:
            ...
            padded_sequences = pad_sequences(
                sequences, padding="post", truncating="post", maxlen=self.max_length
            )
            Y.append(label)
    ...
    def keras_preprocessor(self, text_list: list) -> list:
        ...
        text_preprocessed = pad_sequences(
            sequences, padding="post", truncating="post", maxlen=self.max_length
        )
